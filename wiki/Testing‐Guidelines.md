...
##### 7.1 **ローカルでのコードカバレッジの確認**
ローカル環境でコードカバレッジを確認するには、以下の手順に従ってください：
...
1. **必要なツールのインストール**
   ...
   # Windows
   # WSLを使用するか、Visual Studioの組み込みカバレッジツールを使用
   ```

2. **カバレッジオプションを有効にしてビルド**
    ```bash
...
5. **レポートの確認**
  ...
  生成されたHTMLレポートは build/coverage/index.html にあります。  
  お好みのブラウザで開いて確認できます
  ...
  カバレッジレポートでは、実行された行が緑色で、実行されていない行が赤色で表示されます。  
  ファイルやディレクトリごとのカバレッジ率も確認できます。  
  目標は**80%以上のカバレッジ率**です。
